...
class ProductoService extends IndexedDB {
  ...
  //         const id = parseToIntOrNull(pData.id); // Cambiado de 'ID' a 'id'
  ...
  //           }
  //         };
  //         const fechaCreacion = parseToISODateString(pData["fechaCreacion"]); // Ajustado
  //         const fechaActualizacion = parseToISODateString(pData["fechaActualizacion"]); // Ajustado
  //
  //         if (id === null) {
...
//          const proveedorId = parseToIntOrNull(pData.proveedorid);
//          const cantidad = parseToIntOrNull(pData.cantidad) ?? 0;
//          const stock = parseToIntOrNull(pData.stock) ?? cantidad; // Si stock no está definido, usa cantidad como valor inicial
//          const precio = parseToFloatOrNull(pData.precio) ?? 0;
//          const pvp = parseToFloatOrNull(pData.pvp) ?? 0;
//          const nombre = parseToString(pData.nombre);
//          const categoriaNombre = parseToString(pData.categorianombre);
...
//          );
//          instancia.id = id;
//          instancia.stock = stock; // Usar el stock de Google Sheets
//          return instancia;
//
//        } catch (parseError) {
//          console.error(`[SYNC ROW ${rowIndex}] Error CRÍTICO parseando:`, pData, parseError);
