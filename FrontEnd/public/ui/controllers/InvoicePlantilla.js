...
export class InvoiceTemplate {
    ...
    static generarHTML(factura, esPreview = false) {
        ...
        const detallesHTML = factura.detalles?.map(item => {
            console.log('Tipo de item:', item.constructor.name); `
        <tr>
          <td class="text-center">${item.imagen ? `<img src="${item.imagen}" alt="${item.nombre}" style="width:60px; height:70px; object-fit:cover; border-radius:50%">` : 'N/A'}</td>
          <td class="text-center">${item.nombre}</td>
          <td class="text-center">${item.cantidad}</td>
          <td class="text-right">$${item.precio.toFixed(2)}</td>
          <td class="text-right">$${item.calcularSubtotal().toFixed(2)}</td>
        </tr>
      `}).join('') || '';
