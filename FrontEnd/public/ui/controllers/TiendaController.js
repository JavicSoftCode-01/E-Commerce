...
class TiendaController {
  ...
  startProductPolling() {
    setInterval(async () => {
      if (!this.tiendaSection.classList.contains('hidden')) {
        await this.cargarProductos();
      }
    }, 3000);
  }
  ...
  async handleAddToCartClick(event) {
    ...
    const productoId = parseInt(addToCartButton.dataset.id);
    if (isNaN(productoId)) {
      console.error("Invalid product ID:", addToCartButton.dataset.id);
      alert("Error: ID de producto inválido.");
      return;
    }
    ...
    try {
      ...
      if (producto && producto.stock > 0) {
        ...
        this.actualizarContadorCarrito();
        addToCartButton.textContent = "✓ Agregado";
        setTimeout(() => addToCartButton.textContent = "Agregar al Carrito", 1500);
      } else {
        alert("Lo sentimos, no hay stock disponible para este producto.");
      }
    } catch (error) {
  ...
  async cargarProductos(filters = {}) {
    try {
      ...
      productos.forEach(producto => this.productList.appendChild(this.createProductElement(producto)));
    } catch (error) {
      console.error("Error fetching products:", error);
      this.productList.innerHTML = '<div class="cart-empty">Error al cargar productos</div>';
    }
  ...
  createProductElement(producto) {
    ...
    productElement.innerHTML = `
      ...
      <div class="product-info">
        ...
        <div class="product-price">$${producto.pvp}</div>
        <div class="product-description">${producto.descripcion}</div>
        <button class="btn btn-primary btn-block add-to-cart" data-id="${producto.id}">Agregar al Carrito</button>
      </div>
    `;
    return productElement;
  }
  ...
  async cargarFiltros() {
    ...
    try {
      ...
      categoriaSelect.innerHTML = '<option value="">Todas las categorías</option>';
      categorias.forEach(c => categoriaSelect.innerHTML += `<option value="${c.id}">${c.nombre}</option>`);
      marcaSelect.innerHTML = '<option value="">Todas las marcas</option>';
      marcas.forEach(m => marcaSelect.innerHTML += `<option value="${m.id}">${m.nombre}</option>`);
    } catch (error) {
  ...
  setupFiltroListeners() {
    ...
    searchButton.addEventListener('click', () => this.aplicarFiltros());
    searchInput.addEventListener('keyup', (e) => { if (e.key === 'Enter') this.aplicarFiltros(); });
    filterCategoria.addEventListener('change', () => this.aplicarFiltros());
    filterMarca.addEventListener('change', () => this.aplicarFiltros());
    sortBy.addEventListener('change', () => this.aplicarFiltros());
  }

  async aplicarFiltros() {
    const filters = {
      categoria: document.getElementById('filterCategoria').value ? parseInt(document.getElementById('filterCategoria').value) : null,
      marca: document.getElementById('filterMarca').value ? parseInt(document.getElementById('filterMarca').value) : null,
      search: document.getElementById('searchInput').value,
      sort: document.getElementById('sortBy').value !== 'default' ? document.getElementById('sortBy').value : null
    };
    await this.cargarProductos(filters);
  }
